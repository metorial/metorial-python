...
class MetorialProtoGuardConfigEndpoint(BaseMetorialEndpoint):
    """ProtoGuard config controls prompt-injection filters and alert thresholds."""

    def __init__(self, config: MetorialEndpointManager):
        super().__init__(config)

    def get(self) -> DashboardInstanceProtoGuardConfigGetOutput:
        """
    ...
    def update_filter(self, filter_id: str, *, enabled: Optional[bool] = None, alert_confidence_threshold: Optional[float] = None) -> DashboardInstanceProtoGuardConfigUpdateFilterOutput:
        """
    ...
    """
        ...
        request = MetorialRequest(
            path=['protoguard-config', 'filters', filter_id],
            body=body_dict
        )
        return self._post(request).transform(mapDashboardInstanceProtoGuardConfigUpdateFilterOutput.from_dict)

    def set_alert_filter_count_threshold(self, *, threshold: Optional[float] = None) -> DashboardInstanceProtoGuardConfigSetAlertFilterCountThresholdOutput:
        """
