...
class MetorialDashboardInstanceProviderCategoriesEndpoint(BaseMetorialEndpoint):
    """A category groups providers by function like 'Developer Tools' or 'ERPs'."""

    def __init__(self, config: MetorialEndpointManager):
        super().__init__(config)

    def list(self, instance_id: str, *, limit: Optional[float] = None, after: Optional[str] = None, before: Optional[str] = None, cursor: Optional[str] = None, order: Optional[str] = None) -> DashboardInstanceProviderCategoriesListOutput:
        """
    ...
    """
        ...
        request = MetorialRequest(
            path=['dashboard', 'instances', instance_id, 'provider-categories'],
            query=query_dict
        )
        return self._get(request).transform(mapDashboardInstanceProviderCategoriesListOutput.from_dict)

    def get(self, instance_id: str, provider_category_id: str) -> DashboardInstanceProviderCategoriesGetOutput:
        """
    ...
    """
        request = MetorialRequest(
            path=['dashboard', 'instances', instance_id, 'provider-categories', provider_category_id]
        )
