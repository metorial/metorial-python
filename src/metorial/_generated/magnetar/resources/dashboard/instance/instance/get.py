...
class mapDashboardInstanceInstanceGetOutputProject:
    ...
    @staticmethod
    def to_dict(value: Union[DashboardInstanceInstanceGetOutputProject, Dict[str, Any], None]) -> Optional[Dict[str, Any]]:
        if value is None:
            return None
...
class mapDashboardInstanceInstanceGetOutput:
    ...
    def from_dict(data: Dict[str, Any]) -> DashboardInstanceInstanceGetOutput:
        ...
        created_at=datetime.fromisoformat(data.get('created_at').replace('Z', '+00:00')) if data.get('created_at') else None,
        updated_at=datetime.fromisoformat(data.get('updated_at').replace('Z', '+00:00')) if data.get('updated_at') else None,
        project=mapDashboardInstanceInstanceGetOutputProject.from_dict(data.get('project')) if data.get('project') else None
        )

    @staticmethod
    def to_dict(value: Union[DashboardInstanceInstanceGetOutput, Dict[str, Any], None]) -> Optional[Dict[str, Any]]:
        if value is None:
            return None
        if isinstance(value, dict):
            return value
