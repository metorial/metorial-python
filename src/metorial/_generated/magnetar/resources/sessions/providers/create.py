...
@dataclass
class SessionsProvidersCreateBodyToolFilters:
...
class SessionsProvidersCreateBody:
    ...
    tool_filters: Optional[SessionsProvidersCreateBodyToolFilters] = None


class mapSessionsProvidersCreateBodyToolFilters:
    @staticmethod
    def from_dict(data: Dict[str, Any]) -> SessionsProvidersCreateBodyToolFilters:
        return SessionsProvidersCreateBodyToolFilters(
        tool_keys=data.get('tool_keys', [])
        )

    @staticmethod
    def to_dict(value: Union[SessionsProvidersCreateBodyToolFilters, Dict[str, Any], None]) -> Optional[Dict[str, Any]]:
        if value is None:
            return None
        if isinstance(value, dict):
            return value
...
class mapSessionsProvidersCreateBody:
    ...
    def from_dict(data: Dict[str, Any]) -> SessionsProvidersCreateBody:
        ...
        tool_filters=mapSessionsProvidersCreateBodyToolFilters.from_dict(data.get('tool_filters')) if data.get('tool_filters') else None
        )
