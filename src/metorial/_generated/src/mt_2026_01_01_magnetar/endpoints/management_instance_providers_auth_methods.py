...
class MetorialManagementInstanceProvidersAuthMethodsEndpoint(BaseMetorialEndpoint):
    ...
    """
        ...
        request = MetorialRequest(
            path=['instances', instance_id, 'provider-auth-methods'],
            query=query_dict
        )
        return self._get(request).transform(mapDashboardInstanceProvidersAuthMethodsListOutput.from_dict)

    def get(self, instance_id: str, provider_auth_method_id: str) -> DashboardInstanceProvidersAuthMethodsGetOutput:
        """
    ...
    """
        request = MetorialRequest(
            path=['instances', instance_id, 'provider-auth-methods', provider_auth_method_id]
        )
