...
class MetorialPortalsAuthSsoTenantsConnectionsEndpoint(BaseMetorialEndpoint):
    """Manage the Ares-backed authentication configuration for a portal."""

    def __init__(self, config: MetorialEndpointManager):
        super().__init__(config)

    def list(self, portal_id: str, sso_tenant_id: str, *, limit: Optional[float] = None, after: Optional[str] = None, before: Optional[str] = None, cursor: Optional[str] = None, order: Optional[str] = None) -> DashboardInstancePortalsAuthSsoTenantsConnectionsListOutput:
        """
    ...
    """
        ...
        request = MetorialRequest(
            path=['portals', portal_id, 'auth', 'sso-tenants', sso_tenant_id, 'connections'],
            query=query_dict
        )
