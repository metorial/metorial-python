...
class MetorialManagementInstanceProviderDeploymentsAuthConfigsEndpoint(BaseMetorialEndpoint):
    ...
    """
        ...
        request = MetorialRequest(
            path=['instances', instance_id, 'provider-auth-configs'],
            query=query_dict
        )
    ...
    """
        request = MetorialRequest(
            path=['instances', instance_id, 'provider-auth-configs', provider_auth_config_id]
        )
        return self._get(request).transform(mapDashboardInstanceProviderDeploymentsAuthConfigsGetOutput.from_dict)

    def create(self, instance_id: str, *, name: str, provider_auth_method_id: str, value: Dict[str, Any], description: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None, provider_deployment_id: Optional[str] = None) -> DashboardInstanceProviderDeploymentsAuthConfigsCreateOutput:
        """
