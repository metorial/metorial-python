...
class MetorialDashboardInstanceScmConnectionsEndpoint(BaseMetorialEndpoint):
    """Manage source control connections for an instance."""

    def __init__(self, config: MetorialEndpointManager):
        super().__init__(config)

    def list(self, instance_id: str, *, limit: Optional[float] = None, after: Optional[str] = None, before: Optional[str] = None, cursor: Optional[str] = None, order: Optional[str] = None) -> DashboardInstanceScmConnectionsListOutput:
        """
    ...
    """
        ...
        request = MetorialRequest(
            path=['dashboard', 'instances', instance_id, 'scm', 'connections'],
            query=query_dict
        )
        return self._get(request).transform(mapDashboardInstanceScmConnectionsListOutput.from_dict)

    def get(self, instance_id: str, scm_connection_id: str) -> DashboardInstanceScmConnectionsGetOutput:
        """
    Get SCM connection
    ...
    """
        request = MetorialRequest(
            path=['dashboard', 'instances', instance_id, 'scm', 'connections', scm_connection_id]
        )
        return self._get(request).transform(mapDashboardInstanceScmConnectionsGetOutput.from_dict)

    def create(self, instance_id: str, *, redirect_url: Optional[str] = None) -> DashboardInstanceScmConnectionsCreateOutput:
        """
