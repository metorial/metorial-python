...
class mapOrganizationsProfileGetOutputBadges:
    ...
    @staticmethod
    def to_dict(value: Union[OrganizationsProfileGetOutputBadges, Dict[str, Any], None]) -> Optional[Dict[str, Any]]:
        if value is None:
            return None
...
class mapOrganizationsProfileGetOutput:
    ...
    def from_dict(data: Dict[str, Any]) -> OrganizationsProfileGetOutput:
        ...
        badges=[mapOrganizationsProfileGetOutputBadges.from_dict(item) for item in data.get('badges', []) if item],
        created_at=datetime.fromisoformat(data.get('created_at')) if data.get('created_at') else None,
        updated_at=datetime.fromisoformat(data.get('updated_at')) if data.get('updated_at') else None
        )

    @staticmethod
    def to_dict(value: Union[OrganizationsProfileGetOutput, Dict[str, Any], None]) -> Optional[Dict[str, Any]]:
        if value is None:
            return None
        if isinstance(value, dict):
            return value
