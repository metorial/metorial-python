...
class mapManagementOrganizationTeamsGetOutputProjectsRolesRole:
    @staticmethod
    def from_dict(data: Dict[str, Any]) -> ManagementOrganizationTeamsGetOutputProjectsRolesRole:
    ...
    @staticmethod
    def to_dict(value: Union[ManagementOrganizationTeamsGetOutputProjectsRolesRole, Dict[str, Any], None]) -> Optional[Dict[str, Any]]:
        if value is None:
            return None
        if isinstance(value, dict):
            return value
...
class mapManagementOrganizationTeamsGetOutputProjectsRoles:
    @staticmethod
    def from_dict(data: Dict[str, Any]) -> ManagementOrganizationTeamsGetOutputProjectsRoles:
        return ManagementOrganizationTeamsGetOutputProjectsRoles(
        id=data.get('id'),
        role=mapManagementOrganizationTeamsGetOutputProjectsRolesRole.from_dict(data.get('role')) if data.get('role') else None,
        created_at=datetime.fromisoformat(data.get('created_at').replace('Z', '+00:00')) if data.get('created_at') else None,
        updated_at=datetime.fromisoformat(data.get('updated_at').replace('Z', '+00:00')) if data.get('updated_at') else None
        )

    @staticmethod
    def to_dict(value: Union[ManagementOrganizationTeamsGetOutputProjectsRoles, Dict[str, Any], None]) -> Optional[Dict[str, Any]]:
        if value is None:
            return None
        if isinstance(value, dict):
            return value
...
class mapManagementOrganizationTeamsGetOutputProjects:
    ...
    def from_dict(data: Dict[str, Any]) -> ManagementOrganizationTeamsGetOutputProjects:
        ...
        roles=[mapManagementOrganizationTeamsGetOutputProjectsRoles.from_dict(item) for item in data.get('roles', []) if item]
        )
