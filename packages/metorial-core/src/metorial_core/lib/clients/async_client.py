...
class Metorial(MetorialBase):
  ...
  async def with_session(self, init: Union[Dict[str, Any], str, List[str]], action: Callable[[MetorialSession], Any]):
  ...
  async def with_provider_session(self, provider: Callable[[MetorialSession], Any],
  ...
  async def with_oauth_session(self, oauth_session_id: str, deployment_id: str, action: Callable[[MetorialSession], Any]):
    ...
    try:
      ...
      self.logger.info(f"Creating MCP session with OAuth authentication: {oauth_session_id}")
      ...
      async with httpx.AsyncClient() as client:
        ...
        if response.status_code not in [200, 201]:
          ...
          raise MetorialAPIError(f"Failed to create MCP session with OAuth: {response.status_code}")
  ...
  async def _run_with_oauth(self, message: str, deployment_id: str,
                            provider_client, provider_type: Optional[str], 
                            max_iterations: int, oauth_session_id: Optional[str],
                            oauth_connection_id: Optional[str], oauth_metadata: Optional[Dict[str, Any]],
                            metrics: ChatMetrics) -> str:
    ...
    if oauth_session_id:
      ...
      if not oauth_connection_id:
        ...
        try:
          ...
          if not connections.items or len(connections.items) == 0:
            raise MetorialAPIError("No OAuth connections found. Please create one first or provide oauth_connection_id.")
      ...
      async with httpx.AsyncClient() as client:
        ...
        if response.status_code not in [200, 201]:
          raise MetorialAPIError(f"Failed to create OAuth session: {response.status_code} - {response.text}")
    ...
    result = await self.with_session({"serverDeployments": [deployment_id]}, chat_action)
    
    metrics.end_time = time.time()
    return result  # type: ignore[no-any-return]

  async def run(self, message: str, deployment_id: Union[str, List[str]],
                provider_client, provider_type: Optional[str] = None, max_iterations: int = 5,
                model: Optional[str] = None, tools: Optional[List[Dict[str, Any]]] = None,
                oauth_session_id: Optional[str] = None, oauth_connection_id: Optional[str] = None,
                oauth_metadata: Optional[Dict[str, Any]] = None) -> str:
    ...
    try:
      ...
      if oauth_session_id or oauth_connection_id or oauth_metadata:
        return await self._run_with_oauth(
          message, deployment_id, provider_client, provider_type, 
          max_iterations, oauth_session_id, oauth_connection_id, oauth_metadata, metrics
        )
  ...
  async def run_oauth(self, message: str, deployment_id: str,
                     provider_client, provider_type: Optional[str] = None, 
    ...
    try:
      ...
      if oauth_session_id:
        ...
        if not oauth_connection_id:
          connections = self.provider_oauth.connections.list()  # type: ignore[attr-defined]
          if not connections.items or len(connections.items) == 0:
            raise MetorialAPIError("No OAuth connections found. Create one first or provide oauth_connection_id.")
          oauth_connection_id = connections.items[0].id
          self.logger.info(f"Auto-detected OAuth connection: {oauth_connection_id}")
  ...
  async def _chat_loop_new(self, adapter: ProviderAdapter, messages: List[ChatMessage],
  ...
  async def stream(self, adapter: ProviderAdapter, messages: List[ChatMessage],
                                  max_iterations: int = 10) -> AsyncGenerator[StreamEvent, None]:
    metrics = ChatMetrics(start_time=time.time())
  ...
  async def batch_run(self, messages: List[str], deployment_id: Union[str, List[str]],
    ...
    async def process_single_chat(message: str) -> str:
      return await self.run(message, deployment_id, provider_client, provider_type, max_iterations)
