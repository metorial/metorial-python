...
class MetorialServerConfigVaultsEndpoint(BaseMetorialEndpoint):
  ...
  def list(
    ...
    *,
    ...
    request = MetorialRequest(path=["server-config-vault"], query=query_dict)
    return self._get(request).transform(
      mapDashboardInstanceServerConfigVaultsListOutput.from_dict
    )

  def get(
    self, server_config_vault_id: str
  ) -> DashboardInstanceServerConfigVaultsGetOutput:
    """
    Get server run
    ...
    """
    request = MetorialRequest(path=["server-config-vault", server_config_vault_id])
    return self._get(request).transform(
  ...
  def create(
    ...
    *,
    ...
    request = MetorialRequest(path=["server-config-vault"], body=body_dict)
  ...
  def update(
    ...
    *,
    ...
    request = MetorialRequest(
      path=["server-config-vault", server_config_vault_id], body=body_dict
    )
