...
class MetorialProviderOauthTakeoutsEndpoint(BaseMetorialEndpoint):
  ...
  def list(
    ...
    *,
    ...
    request = MetorialRequest(path=["provider-oauth", "takeouts"], query=query_dict)
  ...
  def create(
    ...
    *,
    ...
    request = MetorialRequest(path=["provider-oauth", "takeouts"], body=body_dict)
    return self._post(request).transform(
      mapDashboardInstanceProviderOauthTakeoutsCreateOutput.from_dict
    )

  def get(self, takeout_id: str) -> DashboardInstanceProviderOauthTakeoutsGetOutput:
    """
    ...
    """
    request = MetorialRequest(path=["provider-oauth", "takeouts", takeout_id])
    return self._get(request).transform(
