...
class MetorialMagicMcpSessionsEndpoint(BaseMetorialEndpoint):
  ...
  def list(
    ...
    *,
    ...
    request = MetorialRequest(path=["magic-mcp-sessions"], query=query_dict)
    return self._get(request).transform(
      mapDashboardInstanceMagicMcpSessionsListOutput.from_dict
    )

  def get(
    self, magic_mcp_session_id: str
  ) -> DashboardInstanceMagicMcpSessionsGetOutput:
    """
    ...
    """
    request = MetorialRequest(path=["magic-mcp-sessions", magic_mcp_session_id])
    return self._get(request).transform(
