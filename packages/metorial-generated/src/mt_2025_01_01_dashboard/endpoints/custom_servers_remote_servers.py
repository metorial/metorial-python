...
class MetorialCustomServersRemoteServersEndpoint(BaseMetorialEndpoint):
  ...
  def list(
    self, query: DashboardInstanceCustomServersRemoteServersListQuery = None
  ) -> DashboardInstanceCustomServersRemoteServersListOutput:
    """
    List remote servers
    ...
    request = MetorialRequest(
      path=["custom-servers", "remote-servers"],
  ...
  def get(
    ...
    """
    request = MetorialRequest(path=["custom-servers", "remote-servers", remoteServerId])
    return self._get(request).transform(
